...
TransportRouter::BuildRoute(TransportCatalogue& transport_catalogue, std::string from,
    std::string to) {
...
void transport_router::TransportRouter::AddVertexes(const Bus& bus)
{
    using namespace std::literals;
    for (const auto& bus_stop : bus.route) {
        //добавляем вершину
        if (stop_to_vertex_.count(bus_stop->stop_name) == 0) {
            stop_to_vertex_[bus_stop->stop_name] = stop_to_vertex_.size();
        }
        //добавляем вершину - ожидание, вдруг здесь будет куда пересаживаться
        if (stop_to_vertex_.count(bus_stop->stop_name + "_wait"s) == 0) {
            stop_to_vertex_[bus_stop->stop_name + "_wait"s] = stop_to_vertex_.size();
            //вершину ожидания добавили - сделаем к нему ребро из остановки
            auto edge_id = graph_->AddEdge(Edge{ stop_to_vertex_.at(bus_stop->stop_name),
                                                stop_to_vertex_.at(bus_stop->stop_name + "_wait"s),
                                                settings_.bus_wait_time + 0.0 });
            edge_to_route_segment_[edge_id] = RouteEdgeInfo{ bus_stop->stop_name, ""s,
...
void transport_router::TransportRouter::AddEdges(const TransportCatalogue& transport_catalogue, const Bus& bus)
{
    ...
    for (auto it_start = bus.route.begin(); it_start != bus.route.end(); ++it_start) {
        ...
        for (auto it_end = std::next(it_start); it_end != bus.route.end(); ++it_end) {
            ...
            auto edge_id = graph_->AddEdge(Edge{ stop_to_vertex_.at((*it_start)->stop_name + "_wait"s),
                                                stop_to_vertex_.at((*it_end)->stop_name),
                                                time_to_drive_forward });
            edge_to_route_segment_[edge_id] = RouteEdgeInfo{ (*it_start)->stop_name, bus.bus_name,
                                                            ++span_count, time_to_drive_forward,
                                                            RouteSegmentType::GoesByBus };

            if (!bus.is_roundtrip) {
                edge_id = graph_->AddEdge(Edge{ stop_to_vertex_.at((*it_end)->stop_name + "_wait"s),
                                               stop_to_vertex_.at((*it_start)->stop_name),
                                               time_to_drive_reverse });
                edge_to_route_segment_[edge_id] = RouteEdgeInfo{ (*it_end)->stop_name, bus.bus_name,
